    ...
    @if ($categories)
        <div class="card card-default mb-3">
            <div class="card-header">
                @if ($category)
                    Categories of {{ $category->name_am }}
                @else
                    Categories
                @endif
            </div>
                        ...
                        <div class="col-md-3">
                            <ul class="list-unstyled">
                                @foreach ($chunk as $current)
                                    <li>
                                        <a href="{{ route('adverts.index', array_merge(['adverts_path' => adverts_path($region, $current)], request()->all())) }}">{{ $current->name_am }}</a>
                    ...
                    @foreach (array_chunk($regions, 3) as $chunk)
                        <div class="col-md-3">
                            <ul class="list-unstyled">
                                @foreach ($chunk as $current)
                                    <li>
                                        <a href="{{ route('adverts.index', array_merge(['adverts_path' => adverts_path($current, $category)], request()->all())) }}">{{ $current->name_am }}</a>
                    ...
                    <div class="advert">
                        <div class="row">
                            ...
                            <div class="col-md-9">
                                ...
                                <p>Region: <a href="">{{ $advert->region ? $advert->region->name_am : 'All' }}</a></p>
                                <p>Category: <a href="">{{ $advert->category->name_am }}</a></p>
